Math Marbling Step 04
=====================

Wavy Pattern Function
--------------------------

在這個版本裡, 使用 Mathematical Marbling 3.1.3 的公式(3) 來進行 梳子拉動(Wavy Pattern Function) 的模擬。拉線效果 公式(3) 如下:

![f(v)=A\sin(\omega\upsilon+\phi)](https://latex.codecogs.com/gif.latex?f(v)=A\sin(\omega\upsilon&plus;\phi))

![\mathbf{P'}=\mathbf{P}+f(\mathbf{P}\cdot[\cos t,-\sin t])[\cos t,\sin t]](https://latex.codecogs.com/gif.latex?\mathbf{P'}=\mathbf{P}&plus;f(\mathbf{P}\cdot[\cos&space;t,-\sin&space;t])[\cos&space;t,\sin&space;t])

與 tine line 很相似, 可看成是 tine line 的加強版。
因為需要重覆審視出來的結果, 所以必須要有 Undo 的能力。
因此, curves 裡的頂點不應該直接修改。應該要可以演練後, 再決定是否要確定。

在這個版本, 花了很多時間去看 `A*cos(2*PI*f+phase)` 產生的變化。最後使用一堆線段,來確認畫出來的 sine wave 正弦波的效果。
...
String [] functionText={"Ink Drop", "Ink Drop Simple", "Tine Line Pattern", "Wavy Pattern"};
int function=0; //0: Ink Drop, 1: Ink Drop Simple, 2: Tine Line Pattern, 3: Wavy Pattern
...
1. 目前的作法, 是在選擇 function=3 之後, 每次 mousePressed 時, 馬上 copy 整個 curves 到 curves2。
...
2. 使用keyboard切換function功能:  
  ...
  - 在 function=3時, mousePressed 需要備份 curves 到 curves2, 在 mouseRelease
  - 在 function=3時, 每次 mousePressed 備份 curves 到 curves2。mouseReleased 時, 暫一個段落。
  - 如果 Wavy Pattern Function 的結果不滿意, 可以按 ESC鍵, 回到備份的 curves2 (但ESC在其他功能會關閉, 需要保護一下)
