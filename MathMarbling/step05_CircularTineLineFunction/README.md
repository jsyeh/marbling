Math Marbling Step 05
=====================

Circular Tine Line Function
---------------------------

在這個版本裡, 使用 Mathematical Marbling 3.1.4 的公式(4) 來進行 圓形拉動(Circular Tine Line Function) 的旋轉模擬。旋轉效果的公式(4) 如下:

![r=radius](https://latex.codecogs.com/gif.latex?r=radius)  
![d=||\mathbf{P}-\mathbf{C}|-r|](https://latex.codecogs.com/gif.latex?d=||\mathbf{P}-\mathbf{C}|-r|)  
...
String [] functionText={"Ink Drop", "Ink Drop Simple", "Tine Line Pattern", "Wavy Pattern", "Circular Tine Line Pattern"};
int function=0; //0: Ink Drop, 1: Ink Drop Simple, 2: Tine Line Pattern, 3: Wavy Pattern, 4: Circular Tine Line Pattern
...
void keyPressed(){ //利用鍵盤來切換功能
  ...
  if(key==ESC && (function==2||function==3||function==4)){//Undo (for Tine Line Pattern or Wavy Pattern)
    key=0;
...
void mousePressed(){ //按下去時, 建新的 curve, 同時暫時把圓心放在裡面
  ...
  if(function==4) alpha4=0;
}
float alpha4=0; //alpha for function 4
void mouseDragged(){
  if(function==2){
...
1. 目前的作法, 是在選擇 function=4 之後, 每次 mousePressed 時, 馬上 copy 整個 curves 到 curves2。
...
2. 使用keyboard切換function功能:  
  ...
  - 在 function=4時, mousePressed 需要備份 curves 到 curves2, 在 mouseRelease
  - 在 function=4時, 每次 mousePressed 備份 curves 到 curves2。mouseReleased 時, 暫一個段落。
  - 如果 Circular Tine Pattern Function 的結果不滿意, 可以按 ESC鍵, 回到備份的 curves2 (但ESC在其他功能會關閉, 需要保護一下)
